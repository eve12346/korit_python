...
class Account:
    def __init__(self,owner,balance=0):
        ...
        print(f"{self.owner}님의 계좌가 개설 되었습니다. 잔액은 {self.balance}원 입니다.")

    def deposit(self,amount):
        if amount > 0:
            self.balance += amount
            print(f"{amount}원 입금 되었습니다. 입금된 후 {self.owner}님의 잔액은 {self.balance}원 입니다")
        else:
            print("입금액은 0보다 커야 합나다.")
    ...
    def withdraw(self, amount):
        if amount <= 0: #출금액이 0보다 작은 상황
            ...
            self.balance -= amount
            print(f"{amount}원만큼 출금 되었습니다. 출금된 후 {self.owner}님의 잔액은{self.balance}원 입니다.")
        else: #출금액이 0이상이면서 잔액보다 출금액이 더 많은 상황
            print(f"잔액이 부족합니다. 현재 잔액:{self.balance}원")

account1 = Account("곽민성",1000000)
