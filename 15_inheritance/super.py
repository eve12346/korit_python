...
class Car(Vehicle):
    def __init__(self, brand, year, color):
        ...
        #이렇게 하면 brand, year의 속성이 Vehicle클래스의 생성자에서 초기화
        super().__init__(brand, year)
        self.color = color #Car클래스 만의 새로운 속성 color를 초기화
        print(f"자동차({self.brand}, {self.year}, {self.color})가 생성되었습니다.")
    ...
    def drive(self):
        print(f"{self.color},{self.color}가 운전 중 입니다.")

#오토바이 클래스
#생성자는 brand, year, type(Cruiser)
#wheelie 메소드 => 브랜드, 색상, 오토바이가 윌리를 합니다!

class Motorcyle(Vehicle):
    def __init__(self, brand, year, type):
        super().__init__(brand,year)
        self.type = type
        print(f"오토바이({self.brand}, {self.year}, {self.type})가 생성되었습니다.")
...
cycle1 = Motorcyle("Honda",2025,"Cruiser")
cycle1.diplay_info()
cycle1.wheeile()
...
#class Manager
#department
#display_info => 부모꺼 호출해서 출력하고 department따로 또 출력
#assign_task(task) => (이름)매니저가 (task)일을 할당 합니다. 출력
...
class Manager(Employee):
    ...
    def assign_task(self, task):
        print(f"{self.name} 메니저가 {task}를 할당 합니다.")
