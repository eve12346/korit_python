...
def btn_click(text):
    # 현재의 entry문자열을 들고와서 변수에 담아둠
    currunt_text = entry.get()

    # entry를 싹 비워줌
    entry.delete(0, tk.END)  # 처음부터 끝까지 다 지움

    # 원래 entry에 들어있던 현재 문자열에다가 새로운 문자 더함
    currunt_text += str(text)

    # entry에 더한 문자열을 다시 넣어준다
    entry.insert(0, currunt_text)
...
tk.Button(root, text="C", font=font.Font(size=28),command=clear).grid(row=4,column=0,ipadx=17)
tk.Button(root, text="=", font=font.Font(size=28),command=equal_sign).grid(row=4,column=3,ipadx=20)
tk.Button(root, text="<-",font=font.Font(size=20)).grid(row=5, column=3,ipadx=20)
