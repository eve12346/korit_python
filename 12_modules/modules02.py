#requests 모듈
#requests는 python에서 HTTP 요청을 보내는 데 사용되는 라이브러리이다
# import requests
# response = requests.get("https://www.naver.com/")
# print(response.status_code) #웹 서버로부터 받은 응답의 HTTP 상태코드를 출력
# print(response.text)

#pandas 모듈
#pandas는 데이터 조작 및 분석을 위한 라이브러리다.
#CSV, Excel, 데이터베이스 등 다양한 형식의 데이터를 읽고 쓸 수 있으며,
#데이터를 정렬, 필터링, 그룹화하는 기능을 재공한다
import pandas as pd
...
#matplotlib
#python에서 정적인, 애니메이션화된, 그리고 상호작용하는 시각화를 만드는데 사용
import matplotlib.pyplot as plt
# df_data.groupby("Age")["Salary"].mean().plot(kind="bar")
# #df_data를 "Age" 칼럼으로 그룹화하고 각 연령대 별로 "Salary"의 평균을 막대 그래프로 그린다.
# plt.title("연령별 평균 연봉") #그래프의 제목
# plt.show() #그래프를 하면에 표시함

#numpy는 python에서 수치 계산, 특히 다차원 배열을 다루는데 핵심적인 라이브러리다.
...
#랜던 다차원 배열
# random_int = np.random.randint(1, 100, 15)
# print(random_int)
